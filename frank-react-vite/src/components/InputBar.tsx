...
type InputBarProps = {
  messageInputRef?: React.RefObject<HTMLInputElement>; // Optional ref for focusing the input
  currentMessage: string;                              // Current text in the input
...
const InputBar: React.FC<InputBarProps> = ({
  ...
  const { aiContext, updateAIContext } = useAuthStore();
  const maxWords = aiContext.maxWords;
  const currentLang = aiContext.lang; // Current language setting

  const setMaxWords = (val: number) => updateAIContext({ maxWords: val });
    ...
    <div className="p-5 bg-white border-t border-gray-200">
      <div className="flex gap-3 items-center flex-wrap">
        ...
        <button
          onClick={clearChat}
          disabled={isLoading}
          title={currentLang === 'IT' ? 'Pulisci chat' : 'Clear chat'}
